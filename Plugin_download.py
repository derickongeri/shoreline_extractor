...
def maths(img):
   vv_vh = img.select('VV').divide(img.select('VH')).rename('VV/VH')
   image = img.addBands(vv_vh)
#    rgbImage = image.visualize({
#         'bands': ['VV', 'VH', 'VV/VH'], # Assign bands to RGB channels
#         'min': -25,  # Adjust these values based on your data
#         'max': 8,
#         'gamma': 1.4
#         })
   return image
